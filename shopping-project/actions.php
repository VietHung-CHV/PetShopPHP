...
if(isset($_POST['addCart'])){
    ...
    $slg = $_POST['sl'];
    $uid = $_POST['uid'];
    $sql1 = "SELECT * FROM product_cart WHERE p_id=$p_id AND user_id = $uid";
    $res1 = mysqli_query($conn1, $sql1);
    $rss =  mysqli_fetch_assoc($res1);
    if (count($rss) < 1) {
        $sql = "INSERT INTO product_cart (p_id, user_id, qty) VALUES ('$p_id', '$uid', '$slg')";
        mysqli_query($conn1, $sql);
    } else {
        // $r = mysqli_fetch_assoc($res1);
        $s = $rss['qty'] + $slg;
        $sql2 = "UPDATE product_cart SET qty = '$s' WHERE p_id=$p_id AND user_id = $uid";
...
if(isset($_POST['removeCartItem'])){
    ...
    $uid = $_POST['uid'];
    mysqli_query($conn1, "DELETE FROM product_cart WHERE p_id='{$p_id}' AND user_id = '{$uid}'");
